...
float th[] = {51.33, 50.26, 49.34, 48.69, 47.87, 47.51, 47.11, 46.90, 46.86, 46.77, 47.66, 48.43, 49.49, 50.80, 52.52, 52.97, 53.36, 53.45};
float eth[] ={0.10, 0.10, 0.10, 0.09, 0.09, 0.09, 0.09, 0.10, 0.09, 0.09, 0.09, 0.09, 0.10, 0.10, 0.10, 0.20, 0.27, 0.15};
...
gsoglia->SetTitle("<Q_{in}>(TPphase)_{40 MHz} con IBiasP_{1} = 200 DAC e IBiasP_{2} = 300 DAC");
...
TF1 *fitQ = new TF1("fitQ","[4]+([0]+[3]*x)/(1-TMath::Exp(-(x-[1])/[2]))",4,25);
...
gfunzionale->SetTitle("F(TPphase)_{40 MHz} con con IBiasP_{1} = 200 DAC e IBiasP_{2} = 300 DAC");
...
TF1 *fitf = new TF1("fitf","([0]-[1]*(TMath::Exp(-[2]/x)))*(1-[3]*x)",4,25);
